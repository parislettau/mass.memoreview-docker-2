...
class Feeds
{
  ...
  private static function generateFeedRss(Pages $p, ?string $collectionName, string $feedTitle, int &$lastMod, bool $debug): string
  {
    ...
    $r .= "    <lastBuildDate>" . date('D, d M Y G:i:s', (int) $lastMod) . " GMT</lastBuildDate>\n";
    $r .= "    <pubDate>" . date('D, d M Y G:i:s', (int) $lastMod) . " +1000</pubDate>\n";
  ...
  private static function addPageToRssFeed(Page $p, int $whenPub, int $whenMod, string &$r, ?bool $isLastPage = false): void
  {
    ...
    $r .= "    <title>" . Xml::encode($p->content()->get('title')) . "</title>\n";
    $r .= "    <link>" . $p->url() . "</link>\n";
    $r .= "    <media:content medium=\"image\" url=\"" . $p->thumbnail()->toFile()->url() . "\"></media:content>\n";
    $r .= "    <author>by " . $p->author() . "</author>\n";
    $r .= "    <artists>Artists mentioned: " . $p->artists() . "</artists>\n";
    $r .= "    <description>\n";



    $content = $p->text()->kirbytext()->value();
    $venue = $p->venue()->kirbytext()->value();
    $opened = $p->opened()->toDate('d M');
    $closed = $p->closed()->toDate('d M Y');
    $author = $p->author()->kirbytext()->value();
    $image = $p->thumbnail()->toFile()->url();

    $c = substr($content, 0, 1000) . " . . ." . "\n";

    $thumbnail = "    <figure><img src=\"" . $image . "\"></img></figure>\n";

    $r .= Xml::encode($thumbnail) . "\n";
    $r .= Xml::encode($venue) . "\n";
    $r .= Xml::encode($opened) . " – " . Xml::encode($closed) . "\n";
    $r .= Xml::encode($author) . "\n";
    $r .= Xml::encode($c) . "\n";
    ...
    if ($countA == 0) {
      $r .= "      <dc:creator>" . $p->author() . "</dc:creator>\n";
    }

    // category
    // enclosure
    $r .= "    <guid isPermaLink=\"true\">" . $p->url() . "</guid>\n";
    $r .= "    <pubDate>" . date('D, j M Y G:i:s', $whenMod) . " +1000</pubDate>\n";
    //    $r .= "    <dc:date>" . date( 'D, j M Y G:i:s', $whenMod ) . " GMT</dc:date>\n";
